...
function CurrentProjectID() {
	...
	return "{B3698D9B-8D4B-412E-A2E5-AFAD2FEE5A23}";
}
...
class cExportBase {
	...
	function CharsetMetaTag() {
		return "<meta http-equiv=\"Content-Type\" content=\"text/html" . ((EW_CHARSET <> "") ? "; charset=" . EW_CHARSET : "") . "\">\r\n";
	}
...
// Get file upload URL
function ew_GetFileUploadUrl($fld, $val, $resize = FALSE) {
	if (!ew_EmptyStr($val)) {
		...
		if ($encrypt) {
			...
			$fn = EW_FILE_URL . "?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_Encrypt($path . $val, $key) .
				"&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight; // Encrypt the physical path
		} else {
			$fn = ew_IsRemote($path) ? $path : ew_UrlEncodeFilePath($path);
			$fn .= ew_UrlEncodeFilePath($val);
		}
		return $fn;
	} else {
		return "";
...
function ew_GetImageUrl($fld, $val, $resize = FALSE, $encrypt = EW_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ew_EmptyStr($val)) {
		if ($encrypt) {
			...
			$fn = EW_FILE_URL . "?t=" . rawurlencode($fld->TblName) . "&fn=" . ew_UrlEncodeFilePath($val) .
...
class cExportWord extends cExportBase {
	...
	function Export() {
		...
		if (!EW_DEBUG_ENABLED && ob_get_length())
			ob_end_clean();
		header('Set-Cookie: fileDownload=true; path=/');
		header('Content-Type: application/msword' . ((EW_CHARSET <> "") ? "; charset=" . EW_CHARSET : ""));
...
class cTable extends cTableBase {
	...
	var $AllowAddDeleteRow = FALSE; // Allow add/delete row
...
class cField {
	...
	function SetVisibility() {
		$this->Visible = $GLOBALS[$this->TblVar]->GetFieldVisibility($this->FldParm());
	}
	...
	function SetDbValueDef(&$rs, $value, $default, $skip = FALSE) {
		if ($skip || !$this->Visible || $this->Disabled) {
			if (array_key_exists($this->FldName, $rs))
				unset($rs[$this->FldName]);
			return;
		}
		switch ($this->FldType) {
			case 2:
...
class cListOptions {
	...
	function Render($Part, $Pos="", $RowCnt="", $ScriptType="block", $ScriptId="", $ScriptClassName="") {
		...
		if ($ScriptId <> "") {
			$this->Write($Part, $Pos, $RowCnt, "block", $ScriptId, $ScriptClassName); // Original block for ew_ShowTemplates
	...
	function CustomScriptTag($ScriptId, $ScriptType, $ScriptClass, $RowCnt="") {
		$id = "_" . $ScriptId;
		if (!ew_EmptyStr($RowCnt))
			$id = $RowCnt . $id;
		$id = "tp" . $ScriptType . $id;
		return "<script id=\"" . $id . "\"" . (!ew_EmptyStr($ScriptClass) ? " class=\"" . $ScriptClass . "\"" : "") . " type=\"text/html\">";
	}

	// Write list options
	function Write($Part, $Pos="", $RowCnt="", $ScriptType="block", $ScriptId="", $ScriptClass="") {
		$this->RowCnt = $RowCnt;
...
class cAdvancedSecurity {
	...
	function GetUserLevelID($UserLevelName) {
		global $Language;
		if (ew_SameStr($UserLevelName, "Anonymous")) {
			return -2;
		} elseif ($Language && ew_SameStr($UserLevelName, $Language->Phrase("UserAnonymous"))) {
			return -2;
		} elseif (ew_SameStr($UserLevelName, "Administrator")) {
			return -1;
...
function Database_Connecting(&$info) {
	...
	if (ew_CurrentUserIP () == "127.0.0.1"  || ew_CurrentUserIP () == ":: 1"  || ew_CurrentHost () == "localhost" ) { // testing on local PC
		...
		$info["db"] = "db_simkop"; // sesuaikan dengan nama database di komputer localhost
	} elseif (ew_CurrentHost () == "simkop.selusin.net") { // setting koneksi database untuk komputer server
		$info["host"] = "mysql.hostinger.co.id";  // sesuaikan dengan ip address atau hostname komputer server
		$info["user"] = "u433254588_simko"; // sesuaikan dengan username database di komputer server
		$info["pass"] = "M457r1P 81"; // sesuaikan deengan password database di komputer server
		$info["db"] = "u433254588_simko"; // sesuaikan dengan nama database di komputer server
	}
}
...
class cUserProfile {
	...
	private function IsSystemAdmin($usr) {
		global $Language;
		return $usr == "" || $usr == EW_ADMIN_USER_NAME || $usr == $Language->Phrase("UserAdministrator");
	}
...
function ew_EmptyStr($value) {
	...
	if (preg_match('/&[^;]+;/', $str)) // Contains HTML entities
		$str = @html_entity_decode($str, ENT_COMPAT | ENT_HTML5, EW_ENCODING);
	$str = str_replace(ew_SameText(EW_ENCODING, "UTF-8") ? "\xC2\xA0" : "\xA0", " ", $str);
	return (trim($str) == "");
}
